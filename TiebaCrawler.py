...
# 使用方法
# python launcher.py --ID 4842388571 --TieziKind 0 --Date 2016-11-12 --Time 00:00 --SortedTop 0 --OnlySender 0 --OnlyPicsUser 0 --DownloadPics 0
import sys, getopt
...
def saveFile(listPrint):
    ...
    for Flag_OnlyPicsUser in range(0, len(fileSaveWithPics)):
        ...
        if Flag_OnlySender==1:
            f = open(NewPath + '/[帖子内容-楼主%s].txt' % fileSaveWithPics[Flag_OnlyPicsUser],'w',encoding='utf-8')
        else:
...
def saveSort(SortedTop):
    ...
    # 写入文件
    f = open(NewPath + '/[Top %d].txt' % SortedTop,'w',encoding='utf-8')
    if Deadline == '':
        f.write('# 统计截止日期：%s\n' % datetime.datetime.now().strftime('%Y-%m-%d %H:%M'))
    else:
        f.write('# 统计截止日期：%s\n' % Deadline)
    ...
    for sortMethodIndex in range(0, 3):
        ...
        for cnt in range(0, SortedTop):
            f.write('----------------------------------------[返回排序标题](#%s)\n### Top：%d\nＩＤ：\t%s\n' % (sortMethod[sortMethodIndex],  cnt+1, listPrint[cnt][0]))
            f.write('性别：\t%s\n' % listPrint[cnt][1])
            f.write('等级：\t%s\n' % listPrint[cnt][2])
            f.write('\n#### 楼层  贡献%d层\n' % listPrint[cnt][6])
            f.write('%s\n' % listPrint[cnt][3])
