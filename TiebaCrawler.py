  ...
  def recode_page_content(self, pagecontent, page):
    ...
    if TIEZIKIND == 0:
      ...
      pattern = re.compile(
          ...
          # ! 更新了贴吧的正则表达式
          r'<div class="l_post j_l_post l_post_bright.*?user_name&quot;:&quot;.*?&quot.*?user_sex&quot;:(.*?),.*?level_id&quot;:(.*?),.*?date&quot;:&quot;(.*?)&quot.*?post_no&quot;:(.*?),.*?<img username="(.*?)".*?clearfix" style="display:;">            (.*?)</div>',
    ...
    if (FLAG_GETREGEXRESULT):
      ...
      #            print NewPath
      file_save = open(
          NEWPATH + '/[RegexResults%s].md' % page,
          'w',
          encoding='utf-8')
      for item in results:
        file_save.write('%s\n' % str(item))
...
def save_file(list_print):
  ...
  for flag_only_picsuser in range(0, len(file_save_withpics)):
    ...
    if FLAG_ONLYSENDER == 1:
      file_save = open(
          NEWPATH + '/[帖子内容-楼主%s].md' %
          file_save_withpics[flag_only_picsuser],
          'w',
          encoding='utf-8')
    else:
      file_save = open(
          NEWPATH + '/[帖子内容-全部%s].md' %
          file_save_withpics[flag_only_picsuser],
          'w',
          encoding='utf-8')
...
def save_sort(sorted_top):
  ...
  # 写入文件
  file_save = open(
      NEWPATH + '/[Top %d].md' % sorted_top, 'w', encoding='utf-8')
  if DEADLINE == '':
    file_save.write('# 统计截止日期：%s\n' %
                    datetime.datetime.now().strftime('%Y-%m-%d %H:%M'))
  else:
